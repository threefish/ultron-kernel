...
/**
 * @author 黄川 huchuc@vip.qq.com copy from nutz
 */
...
public class ClassMetaReader {
	...
	public static Map<String, List<String>> getParamNames(Class<?> klass) throws IOException {
		return buildWithClass(klass).paramNames;
	}

	@SneakyThrows
	public static ClassMeta buildWithClass(Class<?> klass) {
		try (InputStream in = klass.getResourceAsStream("/" + klass.getName().replace('.', '/') + ".class");) {
			return build(in);
		}
	}
	...
	public static ClassMeta build(InputStream in) throws IOException {
		...
		for (int i = 0; i < (constant_pool_count - 1); i++) {
			...
			case 1:// CONSTANT_Utf8:
				...
				dis.readFully(data);
				strs.put(i + 1, new String(data, StandardCharsets.UTF_8));// 必然是UTF8的
				break;
